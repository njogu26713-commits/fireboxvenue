...
export default function Solutions() {
  ...
  return (
    <main className="min-h-screen bg-[#05070b] text-[#f5f1eb]">
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-12" aria-labelledby="products-heading">
        <div className="flex items-end justify-between gap-6 border-b border-white/10 pb-5"><div><span className="font-sans text-[10px] tracking-[0.18em] text-[#6ae4ff]">01 / PRODUCTS</span><h2 id="products-heading" className="mt-3 font-sans text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">PRODUCTS</h2></div><Cpu className="h-7 w-7 text-[#ff5a1f]" /></div>
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {localData.services.map((service, index) => <article key={service.id} className="group border border-white/10 bg-[#090d14]"><div className="relative aspect-[4/3] overflow-hidden bg-[#0d141d]"><img src={service.imageUrl || "/manus-storage/firebox-network-atrium_1879ff5e.png"} alt="" className="h-full w-full object-cover opacity-75 transition duration-500 group-hover:scale-105 group-hover:opacity-100" /><div className="absolute inset-0 bg-gradient-to-t from-[#090d14] via-transparent to-transparent" /><span className="absolute left-4 top-4 font-sans text-[10px] tracking-[0.15em] text-[#d4dde8]">0{index + 1} / PRODUCT</span></div><div className="p-6"><div className="flex items-center justify-between gap-4 font-sans text-[10px] tracking-[0.14em] text-[#ff5a1f]"><span>FIREBOX TECH</span><CardLinks label={service.title} liveUrl={service.liveUrl} githubUrl={service.githubUrl} /></div><h3 className="mt-4 font-sans text-2xl font-semibold tracking-[-0.04em]">{service.title}</h3><p className="mt-3 font-sans text-xs leading-6 text-[#9eabbc]">{service.description}</p></div></article>)}
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-5 pb-24 sm:px-8 lg:px-12" aria-labelledby="services-heading">
        <div className="flex items-end justify-between gap-6 border-b border-white/10 pb-5"><div><span className="font-sans text-[10px] tracking-[0.18em] text-[#6ae4ff]">02 / SELECTED SERVICES</span><h2 id="services-heading" className="mt-3 font-sans text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">SERVICES</h2></div><Layers3 className="h-7 w-7 text-[#ff5a1f]" /></div>
