...
${JSON.stringify(knowledge)}`,
        ...
        if (!content) {
          ...
          answer = !needsKnowledge
            ? answer
            : matches.length > 0
            ? `Based on your question, **${matches[0].title}** looks like the most relevant option. ${matches[0].description} ${matches.length > 1 ? `You may also want to look at **${matches[1].title}**: ${matches[1].description}` : ""}`
            : "I couldn’t find enough public information to answer that confidently. Tell me a little more about what you’re trying to do, and I’ll guide you to the right Firebox option.";
        }
